...
class FindObject:
    def __init__(self):
        ...
        self.forwardImage = ForwardImage(
            input_camera_raw = rosparamOrDefault('~camera_raw', '/d435_sim/color/image_raw'),
            input_camera_info = rosparamOrDefault('~camera_info', '/d435_sim/color/camera_info'),
            output_camera_raw = rosparamOrDefault('~dope_camera_raw', '/dope/webcam/image_raw'),
            output_camera_info = rosparamOrDefault('~dope_camera_info', '/dope/webcam/camera_info')
        )
    ...
    def onObjectRequest(self, request):
        ...
        #while not self.dopeCollection.isComplete():
            #rospy.sleep(0.1)
        for i in range(200):
            if not self.dopeCollection.isComplete():
                rospy.sleep(0.1)
