...
class BinPickingSequencer():
    ...
    def sequence(self):
        objects_to_request = ['gelatin','TomatoSauce']
        drop_sites = ['above_bin','drop_location']
        for object_to_request,drop_at in zip(objects_to_request,drop_sites):
	    ...
	    _ = self.rg2_gripper_control_val(110)
            ...
            if request.found_object:
                ...
                if not self.planner.planAndExecuteInReferenceFrame(request.object_pose):
                    ...
                    rospy.sleep(2)
                    rospy.loginfo('Going to drop site')
                    self.planner.planAndExecuteNamedTarget(drop_at)
